...
function MapRoute({ start, end,width,height,isvalid }) {
  useEffect(() => {
    if (!start || !end) return;
    ...
    L.marker(start, { icon: startIcon }).addTo(map).bindPopup("Départ").openPopup();
    L.marker(end, { icon: endIcon }).addTo(map).bindPopup("Arrivée");
